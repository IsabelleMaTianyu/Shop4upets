...
class ImageServer2
{
    ...
    private $thumbsize            = 500;          // max width / height of thumbnail images
    ...
    public function upload()
    {
        ...
        $f3->set('CurrentImageServer2', $this); // so we can call receiveCallback
        ...
        Web::instance()->receive('ImageServer2::receiveCallback', $this->shouldOverwriteFiles, $slug);
        ...
        $filedata['price']     = $f3->get('POST.picprice');
        $filedata['detail']     = $f3->get('POST.detail');
        $filedata['size']     = $f3->get('POST.size');
        $filedata['weight']     = $f3->get('POST.weight');
        $filedata['materials']     = $f3->get('POST.materials');
    ...
    {
        global $f3;
        $imageServer2 = $f3->get('CurrentImageServer2'); // current image server object

        $imageServer2->filedata = $file;        // export file data to outside this function

        if ($imageServer2->filedata['size'] > (2 * 1024 * 1024)) { // if bigger than 2 MB
            $imageServer2->uploadResult = ImageServer2::setUploadFailMessage("File > 2MB");
            return false; // this file is not valid, return false will skip moving it
        }
        if (!in_array($imageServer2->filedata['type'], $imageServer2->acceptedTypes)) {        // if not an approved type
            $imageServer2->uploadResult = ImageServer2::setUploadFailMessage("File type not accepted");
            return false; // this file is not valid, return false will skip moving it
        }
        $imageServer2->uploadResult = 'success'; // everything went fine, hurray!
    ...
    public function store($filedata)
    {
        ...
        $mapper->price      = $filedata['price'];
        $mapper->details      = $filedata['details'];
        $mapper->size      = $filedata['size'];
        $mapper->weight      = $filedata['weight'];
        $mapper->materials      = $filedata['materials'];
    ...
    public function findtitle($titlefind)
    {
        global $f3;            // because we need f3->get()
        $mapper            = new DB\SQL\Mapper($f3->get('DB'), $this->table);
        $list =  $mapper->find(["title LIKE ?", $titlefind]);// load DB record matching the given ID
        return $list;
    }
