...
class Imagesocial
{
    ...
    public function upload()
    {
        ...
        $f3->set('CurrentImageSocial', $this); // so we can call receiveCallback
        ...
        Web::instance()->receive('ImageSocial::receiveCallback', $this->shouldOverwriteFiles, $slug);
    ...
    {
        global $f3;
        $imageSocail = $f3->get('CurrentImageSocial'); // current image server object

        $imageSocail->filedata = $file;        // export file data to outside this function

        if ($imageSocail->filedata['size'] > (2 * 1024 * 1024)) { // if bigger than 2 MB
            $imageSocail->uploadResult = ImageSocail::setUploadFailMessage("File > 2MB");
            return false; // this file is not valid, return false will skip moving it
        }
        if (!in_array($imageSocail->filedata['type'], $imageSocail->acceptedTypes)) {        // if not an approved type
            $imageSocail->uploadResult = ImageSocail::setUploadFailMessage("File type not accepted");
            return false; // this file is not valid, return false will skip moving it
        }
        $imageSocail->uploadResult = 'success'; // everything went fine, hurray!
    ...
    public function showImage($id, $thumb1)
    {
        global $f3;
        $mapper = new DB\SQL\Mapper($f3->get('DB'), $this->table); // create DB query mapper object
        $mapper->load(['id=?', $id]);                              // load DB record matching the given ID
        $fileToShow = (($thumb1) ? $mapper['thumbnail'] : $mapper['filepath']);
        $fileType   = (($thumb1) ? 'image/jpeg' : $mapper['type']);    // thumb is always jpeg
